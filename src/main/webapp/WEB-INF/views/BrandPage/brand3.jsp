		...
		<form action="brand3" method="post">
			<c:if test="${searchVO.totPage>1 }"><!-- 토탈 페이지가 1보다 크면 -->
				<c:if test="${searchVO.page>1 }">
					<a href="brand3?page=1" style="color: black; text-decoration: none">[처음]</a>
					<a href="brand3?page=${searchVO.page-1 }" style="color: black; text-decoration: none">[이전]</a>
				</c:if>
				<c:forEach begin="${searchVO.pageStart }" end="${searchVO.pageEnd }" var="i">
					<c:choose>
						...
						<c:otherwise>
							<a href="brand3?page=${i }" style="color: black; text-decoration: none">${i }</a>&nbsp;
						</c:otherwise>

					</c:choose>
				</c:forEach>
				<c:if test="${searchVO.totPage>searchVO.page }"> <!-- 마지막 페이지가 아닐때 -->
					<a href="brand3?page=${searchVO.page+1 }" style="color: black; text-decoration: none">[다음]</a>
					<a href="brand3?page=${searchVO.totPage }" style="color: black; text-decoration: none">[마지막]</a>
				</c:if>
			</c:if>
		</form>
	</div>
