...
<body>
	...
	<div id="__next" data-reactroot="" class="css-brand">
		...
		<section style="background-image: url(resources/brandimg/brownyard/brownyard.jpeg)" class="css-background-img">

			<div class="css-brand-text-1">

				<div class="css-brand-text-2">

					<h3 class="css-brand-text-3">BROWNYARD</h3>

					<strong class="css-brand-text-4">브라운야드</strong>

					<div class="css-brand-text-5">

						<div class="css-brand-text-6">

							<div class="css-brand-text-7">다양한 문화적 요소를 새로운 시각으로 
								재구성해 안정적인 밸런스를 중점으로 전개하는 브랜드입니다.</div>

						</div>

						<div class="css-brand-text-8">다양한 문화적 요소를 새로운 시각으로 
								재구성해 안정적인 밸런스를 중점으로 전개하는 브랜드입니다.</div>
	...
	<div class="products">
        ...
        <div id="div1" style= "text-align: center;">
	        <table class="product-list">
	        	<tr>
					<c:forEach items="${brand4 }" var="dto" varStatus ="status">
	...
	<c:if test="${ ereginum eq '001-05-43655' }">
			<div>
	...
	<div class="paging" style="text-align:center; padding-bottom: 50px; color: black; text-decoration: none;">
		<form action="brand4" method="post">
			<c:if test="${searchVO.totPage>1 }"><!-- 토탈 페이지가 1보다 크면 -->
				<c:if test="${searchVO.page>1 }">
					<a href="brand4?page=1" style="color: black; text-decoration: none">[처음]</a>
					<a href="brand4?page=${searchVO.page-1 }" style="color: black; text-decoration: none">[이전]</a>
				</c:if>
				<c:forEach begin="${searchVO.pageStart }" end="${searchVO.pageEnd }" var="i">
					<c:choose>
						...
						<c:otherwise>
							<a href="brand4?page=${i }" style="color: black; text-decoration: none">${i }</a>&nbsp;
						</c:otherwise>

					</c:choose>
				</c:forEach>
				<c:if test="${searchVO.totPage>searchVO.page }"> <!-- 마지막 페이지가 아닐때 -->
					<a href="brand4?page=${searchVO.page+1 }" style="color: black; text-decoration: none">[다음]</a>
					<a href="brand4?page=${searchVO.totPage }" style="color: black; text-decoration: none">[마지막]</a>
				</c:if>
			</c:if>
		</form>
	</div>
